...
- Negociación de all-in: voto 1× / 2× / 3× — modal automático + chip flotante reabridor
...
- [x] Mid-game joins
- [x] All-in voting (1/2/3×) con chip flotante
...
#### Juego / lógica
- [ ] **Stats por sala en Firestore** — actualmente solo localStorage del host; migrar `useStats`/`useHistory` a subcolecci Firestore
- [ ] **Torneo: ranking final UI** — `finalRanking[]` y `knockouts[]` ya existen en `TournamentState`; falta pantalla de podio
- [ ] **Torneo: payouts editables** — campo `payouts` en modelo; falta configurador UI en HostDock
- [ ] **Torneo: pago de premios** — botón "Distribuir premios" al terminar que ajuste stacks según `payouts`
- [ ] **Rebuy flow completo en torneo** — modelo de `stackRequests` + `reentries` existe; falta lógica de límite de re-entradas y UI de historial
- [ ] **Transferir host** — si el host cierra la pestaña el juego se congela; añadir botón "Transferir host" en AdminPanel y escuchar `hostUid` en Firestore
- [ ] **Spectator role** — unirse como espectador sin seat; recibe el estado público pero sin hole cards ni betting dock
- [ ] **Notas por seat** — campo de texto por seat visible solo al host; útil para registrar reads o HUDs físicos

#### UX / móvil
- [ ] **Vibración en turno propio** — `navigator.vibrate([200])` cuando `isMyTurn` cambia a `true`; batería-friendly (pulso corto único)
- [ ] **Hand strength en vista de jugador** — mostrar la categoría de tu propia mano (`"Par de ases"`) en el BettingDock; no viola privacidad (solo tus cartas)
- [ ] **Export historial** — botón "Exportar" en tab Historial del HostDock que descarga JSON/TXT con las manos de la sesión
- [ ] **Replay de última mano** — snapshot de la mano completa (calles + acciones) guardado en Firestore con TTL de 1h; viewer con controles prev/next
- [ ] **Canal de voz en modo Presencial** — actualmente solo en `/play/normal/[code]`; llevar VoicePanel a `/play/[code]` también

#### Infraestructura / seguridad
- [ ] **Auditoría de `firestore.rules`** — revisar que el host no pueda sobreescribir datos de otros; añadir rate limiting en writes de `pendingAction`
- [ ] **TURN server** — sin TURN las llamadas fallan en NAT simétrico (redes corporativas, algunos ISP de LatAm); documentar en `voice-setup.md` cómo levantar Coturn en VPS
- [ ] **Optimización de bundle** — analizar con `ANALYZE=true next build`; candidatos a lazy-load: `canvas-confetti`, `ogl`, DiceBear

#### Calidad / cobertura
- [ ] **Tests e2e de showdown** — simular mano completa desde `startHand` hasta ganador con side pots y multi-run
- [ ] **Test de integración Firestore** — usar Firebase Emulator Suite en CI para smoke-test de `rooms.ts` y `normalRooms.ts`
- [ ] **Monitoreo de errores** — integrar Sentry (o similar) para capturar excepciones en producción

#### Plataforma
- [ ] **PWA** — `manifest.json` + service worker (Next.js `next-pwa` o propio); permite instalar en pantalla de inicio en iOS/Android
- [ ] **Accesibilidad** — auditoría a11y: focus rings en botones custom (`btn-press`), `aria-live` para el turno activo, contraste en textos `text-zinc-500`
- [ ] **i18n** — todo hardcodeado en español; si se quiere inglés usar `next-intl` con locale routing
...
- Ampliar cobertura de tests (side pots multi-way, showdown end-to-end)
- Auditoría de seguridad de `firestore.rules` para normal/torneo
