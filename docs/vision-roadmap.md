...
| 2026-06-11 | **El modo online se renderiza en la mesa rica (`TableShell`/`RoundPokerTable` + `BettingDock`); `ServerTable` (vista plana) se elimina.** Adaptador puro `src/lib/onlineTable.ts` (PublicState → view-model); el protocolo gana `startStack`, `currentBet`, `minRaise`, `dealer`, `owner`, `lastAction`, `seed`, `totalBet`. | Una sola experiencia de mesa en toda la app; el cliente sigue sin reglas de juego (solo reshape). Decisión del dueño: la vista plana no se usa. |
| 2026-06-11 | **Economía online cerrada server-side.** Cash-out lee el stack final del Go (`GET /stacks`, con `departed` para quien ya cerró el WS); `record-session mode:"online"` cuenta manos verificadas de Supabase; buy-in rechaza cuentas anónimas; `pagehide` + fetch keepalive liquidan al cerrar la pestaña. | El bug conocido "cash-out no usa el stack del Go" y el XP/historial faltante del online quedan resueltos sin confiar en el cliente. |
| 2026-06-11 | **Entrada observer-first con fila (estilo Home Games).** Se entra a la sala viendo mesa y jugadores sin formularios; "Sentarme" sienta o encola (cupo 9, orden de llegada estable via `JoinSeq`, promoción automática al liberarse asiento). Invitados observan; sentarse pide cuenta. | Cero fricción de entrada y cupo claro; el orden de llegada estable también arregla que el botón del dealer saltara entre manos (antes el roster venía de un map sin orden). |
| 2026-06-11 | **Salas Go efímeras con gracia de 2 min + stacks `departed`.** Sala vacía se desmonta (timers, ticker de ciegas, estado) tras 2 min; quien se levanta deja su stack leíble para el cash-out y un rejoin arranca de cero (un stack ya pagado no resucita). | Sin fugas de memoria/goroutines en Render y sin doble pago: la economía y el ciclo de vida de la sala quedan consistentes. |
